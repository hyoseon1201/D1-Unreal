...
- **실행 배치파일** (`D:\D1\`): `RunTownServer.bat` (port 7777), `RunDungeonServer.bat` (port 7778), `RunHuntingGroundServer.bat` (port 7779), `RunClients.bat` (클라 2개), `RunClient1.bat`, `RunTestBots.bat`(부하테스트용 헤드리스 봇 클라이언트, 아래 9번 참고), `RunHuntingGroundServerDebug.bat`(클라 exe를 `?Listen` URL 옵션으로 띄우는 화면 있는 리슨서버 — 콘솔에서 `show Collision`으로 서버 권한 콜리전 시각 확인 가능), `RunHuntingGroundServerInsights.bat`(`-trace=cpu,frame,counters,stat,bookmark,net` 옵션으로 띄워서 Unreal Insights 프로파일링용 `.utrace` 기록).
...
| 06-18 | **크로스 프로세스 travel (DB 핸드오프) 구현.** Spring Boot: `POST /api/server/sessions/issue`(API Key, CharacterId+destination→세션토큰+서버주소), `JwtUtil::generateSessionTokenForServer`(accountId 불필요), `dungeon-address` config 추가. 언리얼: `UD1HttpSubsystem::IssueSessionToken`, `FD1IssueSessionDelegate`. `AD1GameModeTown::StartDungeonForParty`: `bCrossProcess`(DB로그인 여부)로 분기 — 크로스프로세스는 파티원 각자 SaveCharacter+IssueToken+ClientTravel(개별 비동기), PIE는 기존 GameInstance+ServerTravel 폴백. `AD1PlayerController::Server_ReturnToTown_Implementation` 동일 패턴으로 교체(던전→마을). |
